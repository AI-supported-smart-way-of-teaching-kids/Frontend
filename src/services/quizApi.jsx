...
const convertQuizToSnakeCase = (quiz) => {
  ...
  delete payload.id; // Let backend generate ID for new quizzes
  return payload;
};
...
export const createQuiz = async (quizData) => {
  try {
    const payload = convertQuizToSnakeCase(quizData);
...
export const submitQuizAttempt = async (attemptData) => {
  try {
    const response = await api.post("/quizzes/attempts/submit/", attemptData);
    const submittedAttempt = response.data;
    
    // Update local cache
    const cached = await AsyncStorage.getItem(STORAGE_ATTEMPTS);
    const attempts = cached ? JSON.parse(cached) : [];
    const index = attempts.findIndex(a => a.id === submittedAttempt.id);
    if (index >= 0) {
      attempts[index] = submittedAttempt;
    } else {
    ...
    console.warn("Failed to submit quiz attempt on backend:", error);
    throw error;
  }
};
